...
async function handleTiingoRequest(request) {
  ...
  if (!token) {
    ...
    console.error(`[tiingo] ${symbol}(${kind}): ${warning}. Candidate keys: ${TIINGO_TOKEN_CANDIDATE_KEYS.join(', ')}`);
    const errorDetails = {
      error: 'TIINGO_API_KEY_MISSING',
      message: 'Tiingo API key not configured',
      details: {
        reason: 'missing_token',
        envKeysChecked: TIINGO_TOKEN_CANDIDATE_KEYS,
        solution: 'Set TIINGO_KEY environment variable with your API key from tiingo.com',
  ...
  try {
    if (kind === 'intraday_latest') {
      ...
      return new Response(JSON.stringify({
        error: 'INTRADAY_LATEST_UNAVAILABLE',
        message: 'Real-time data unavailable for this symbol',
        details: {
          symbol,
          kind,
          reason: 'intraday_latest_unavailable_and_no_eod',
          attempted: ['intraday_latest', 'eod_fallback'],
          solution: 'Check if symbol is valid or try again later',
          timestamp: new Date().toISOString()
        }
      }), {
        status: 503,
    ...
    if (kind === 'intraday') {
      ...
      return new Response(JSON.stringify({
        error: 'INTRADAY_DATA_UNAVAILABLE',
        message: 'Intraday chart data unavailable for this symbol',
        details: {
          symbol,
          kind,
          interval,
          ...
          attempted: ['intraday', 'eod_fallback'],
          solution: 'Check if symbol supports intraday data or try daily (eod) data',
          timestamp: new Date().toISOString()
        }
      }), {
        status: 503,
    ...
    if (kind === 'news') {
      ...
      return new Response(JSON.stringify({
        ...
        message: 'Company news unavailable for this symbol',
        details: {
          symbol,
          kind,
          limit,
    ...
    if (kind === 'documents') {
      ...
      return new Response(JSON.stringify({
        error: 'DOCUMENTS_DATA_UNAVAILABLE',
        message: 'Company filings unavailable for this symbol',
        details: {
          symbol,
          kind,
          limit,
    ...
    if (kind === 'filings') {
      ...
      return new Response(JSON.stringify({
        error: 'FILINGS_DATA_UNAVAILABLE',
        message: 'SEC filings unavailable for this symbol',
        details: {
          symbol,
          kind,
          limit,
          reason: 'filings_unavailable',
          solution: 'Check if symbol is valid or try again later',
          timestamp: new Date().toISOString()
        }
      }), {
    ...
    if (kind === 'fundamentals') {
      ...
      return new Response(JSON.stringify({
        error: 'FUNDAMENTALS_DATA_UNAVAILABLE',
        message: 'Fundamentals data unavailable for this symbol',
        details: {
          symbol,
          kind,
          limit,
    ...
    if (kind === 'actions') {
      ...
      return new Response(JSON.stringify({
        error: 'CORPORATE_ACTIONS_UNAVAILABLE',
        message: 'Corporate actions data unavailable for this symbol',
        details: {
          symbol,
          kind,
    ...
    if (kind === 'overview') {
      ...
      return new Response(JSON.stringify({
        error: 'COMPANY_OVERVIEW_UNAVAILABLE',
        message: 'Company overview data unavailable for this symbol',
        details: {
          symbol,
          kind,
    ...
    if (kind === 'statements') {
      ...
      return new Response(JSON.stringify({
        error: 'FINANCIAL_STATEMENTS_UNAVAILABLE',
        message: 'Financial statements data unavailable for this symbol',
        details: {
          symbol,
          kind,
          limit,
          reason: 'statements_unavailable',
          solution: 'Check if symbol is valid or try again later',
          timestamp: new Date().toISOString()
        }
      }), {
    ...
    if (kind === 'valuation') {
      ...
      return new Response(JSON.stringify({
        ...
        message: 'Valuation snapshot unavailable for this symbol',
        details: {
          symbol,
          kind,
    ...
    return new Response(JSON.stringify({
      error: 'EOD_DATA_UNAVAILABLE',
      message: 'End-of-day data unavailable for this symbol',
      details: {
        symbol,
        kind,
        limit,
        reason: 'eod_unavailable',
        solution: 'Check if symbol is valid or try again later',
        timestamp: new Date().toISOString()
      }
    }), {
    ...
    console.error(`[tiingo] ${symbol}(${kind}): FULL ERROR DETAILS:`, {
      error: err.message,
      stack: err.stack,
      code: err.code,
      cause: err.cause,
    ...
    return new Response(JSON.stringify({
      error: 'TIINGO_REQUEST_FAILED',
      message: 'Tiingo API request failed',
      details: {
        symbol,
        kind,
        limit,
        ...
        networkError: err.cause ? String(err.cause) : undefined,
        solution: 'Check network connection, API key, and symbol validity',
        timestamp: new Date().toISOString()
      }
    }), {
      status: 500,
